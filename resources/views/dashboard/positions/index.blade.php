@extends('layouts.app')
@section('title', isset($isTrash) ? 'Position Management - Trash' : 'Position Management')
            ...
            <div class="card mb-4 shadow-sm">
                <div
                    ...
                    <h5 class="mb-0 fw-bold text-primary fs-5 fs-md-4">
                        <i class="fas {{ isset($isTrash) ? 'fa-trash-alt' : 'fa-briefcase' }} me-2"></i>
                        {{ isset($isTrash) ? 'Positions Deleted' : 'Positions Management' }}
                    </h5>
                        ...
                        <tbody>
                            @foreach ($positions as $position)
                                <tr>
                                        ...
                                        <div class="btn-group shadow-sm rounded-pill overflow-hidden border">
                                            @if (isset($isTrash))
                                                @if (auth()->user()->isAdmin() || auth()->user()->role == 'HR')
                                                    ...
                                                    <button type="button" class="btn btn-white btn-sm px-3"
                                                        ...
                                                        data-coreui-target="#forceDeleteModal{{ $position->id }}"
                                            ...
                                            @else
                                                ...
                                                @if (auth()->user()->isAdmin() || auth()->user()->role == 'HR')
                                                    ...
                                                    <a href="{{ route('positions.export', $position->id) }}"
                                                        class="btn btn-white btn-sm px-3" title="Export">
                                                        <i class="fas fa-download text-primary"></i>
                                                    </a>
                                                    <button type="button" class="btn btn-white btn-sm px-3"
                                                        data-coreui-toggle="modal"
                                                        data-coreui-target="#deleteModal{{ $position->id }}"
                                                        title="Delete">
                                    ...
                                    @if (isset($isTrash))
                                        <x-modal id="forceDeleteModal{{ $position->id }}" title="Permanently Delete"
                                            type="danger" :actionUrl="route('positions.force-delete', $position->id)" method="DELETE"
                                    ...
                                    @else
                                        <x-modal id="deleteModal{{ $position->id }}" title="Delete Confirmation"
                                            type="danger" :actionUrl="route('positions.destroy', $position->id)" method="DELETE"
                                            confirmText="Move to Trash">
                                            <div class="text-center py-3">
                                                <i class="fas fa-trash-alt text-danger fa-3x mb-3"></i>
                                                <h5 class="fw-bold">Delete Position?</h5>
                                                <p class="text-muted mb-0">Are you sure you want to move
                                                    <strong>{{ $position->name }}</strong> to trash?
                                                </p>
                                            </div>
                                        </x-modal>
                                    @endif
